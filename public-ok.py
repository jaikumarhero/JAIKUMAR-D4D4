...
#JAIKUMAR 
	print('[[\x1b[1;92mтАв\x1b[1;97m] [\x1b[1;97mMR-JAIKUMAR-XD]')
...
def passwrd():
	...
	print(f'                    {m}J {k}A {h}A{u} {b}H {u}    {b}K {u}U {h} I{k} X{m} I{b} X{h}')
	print('')
	...
	print(f'[{h}JAIKUMAR{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}JAIKUMAR{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}JAIKUMAR>{P}[{k}{loop}{P}/{h}{len(id)}{P}]OK{P}[{H}{ok}{P}]CK{P}[{k}{cp}{x}]KST[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
